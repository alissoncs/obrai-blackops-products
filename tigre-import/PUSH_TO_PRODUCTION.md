# Push Tigre products to production API

This uses [`push_to_production.py`](push_to_production.py) to send `output/tigre_products.json` to the Obrai admin **bulk import** endpoints (`POST /api/admin/products/import` and `POST /api/admin/products/import/images`).
...
- In **production**, ensure the **brand** exists and its name matches `PRODUCTION_BRAND_NAME` in `push_to_production.py`.
...
Process more rows (`--limit` must be between **1** and **1000**):

```bash
python push_to_production.py --limit 100
python push_to_production.py --limit 1000
```
...
```bash
python push_to_production.py --json-path ./output/tigre_products.json \
...
- **State file** (default `output/aux/push_prod_state.json`): tracks per-SKU `import_ok` and `images_ok` / `images_skipped` so you can resume; use `--force-import` / `--force-images` to ignore it.
...
- JSON rows are adapted for bulk v1: nested `supplierProducts` / `attributes` / `variations` are not sent; `pricingUnit` defaults to `UNIT` (override with `--pricing-unit`); placeholder category UUIDs are omitted.
- Bulk JSON is sent in chunks of up to **500** rows per request; up to **20** image files per SKU per request.

## See also

- Main Tigre scraper docs: [README.md](README.md)
